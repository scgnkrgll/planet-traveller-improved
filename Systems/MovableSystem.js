...
class MovableSystem extends System {
    execute(delta, time) {
        this.queries.moving.results.forEach(entity => {
            ...
            if (position.x > canvasWidth + constants.SHAPE_HALF_SIZE) position.x = - constants.SHAPE_HALF_SIZE
            if (position.x < - constants.SHAPE_HALF_SIZE) position.x = canvasWidth + constants.SHAPE_HALF_SIZE
            if (position.y > canvasHeight + constants.SHAPE_HALF_SIZE) position.y = - constants.SHAPE_HALF_SIZE
            if (position.y < - constants.SHAPE_HALF_SIZE) position.y = canvasHeight + constants.SHAPE_HALF_SIZE
        })
        ...
        this.queries.attractors.results.forEach(attractorEntity => {
            ...
            this.queries.attractable.results.forEach(attractableEntity => {
                ...
                const acceleration = attractableEntity.getMutableComponent(Acceleration)
